 ...
 * 
 * The virus class holds all of its own functions such as updating its location, the coordinate it is headed to,
 * drawing itself and getter methods.
 * 
 * @author alexm
 *
 */
public class Virus implements Locatable{
    ...
    /**
     * constructor, takes in the intial x and y coordinate, and instantiates variables
     * 
     * @param x x coord
     * @param y y coord
     */
    ...
    /**
     * when the virus reaches its assigned location this method makes the virus move to prevent them from overlapping
     */
    ...
    /**
     * moves the virus, checks if it has reached the designated location, and if there is anything to attack in 
     * its attack range
     * 
     * @param g draws lines to show attack using graphics
     * @param xOffset the offset to render the object on the screen according to how the screen had shifted
     * @param yOffset same as above
     */
    public void update(Graphics g, int xOffset, int yOffset) {
    	if( (vx < 0 && x < xL) || (vy < 0 && y < yL) || (vy > 0 && y > yL) || (vx > 0 && x > xL) ) {
    		move = false;
    	}
    	if(!move) {
    		idleMovement();
